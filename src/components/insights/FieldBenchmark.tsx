...
export function FieldBenchmark({ faculty }: { faculty: Array<Faculty> }) {
  ...
  const {
    ...
    setData,
    tooltipRef,
  } = useChartTooltip<SchoolBenchmark>()
    ...
    for (const [school, members] of buckets) {
      ...
      for (const f of members) {
        if (f.openalexField)
          fieldCounts.set(
            f.openalexField,
            (fieldCounts.get(f.openalexField) ?? 0) + 1,
          )
        if (f.bestH != null) hs.push(f.bestH)
      }
      let domField = ''
      let maxC = 0
      for (const [field, c] of fieldCounts)
        if (c > maxC) {
          domField = field
          maxC = c
        }
  ...
  useLayoutEffect(() => {
    ...
    const maxH =
      Math.max(...rows.map((r) => Math.max(r.sluMedian, r.p90))) * 1.1
    ...
    // Rows
    const rowG = g
      .selectAll<SVGGElement, SchoolBenchmark>('g.row')
    ...
    // SLU median marker
    rowG
      .selectAll('circle.slu')
      .data((d) => [d])
      .join('circle')
      .attr('class', 'slu')
      .attr('cx', (d) => x(d.sluMedian))
      .attr('cy', 0)
      .attr('r', 6)
      .attr('fill', (d) =>
        d.sluMedian >= d.p75
    ...
    // SLU label
    rowG
      .selectAll('text.slu-label')
      .data((d) => [d])
      .join('text')
      .attr('class', 'slu-label')
      .attr('x', (d) => x(d.sluMedian))
      .attr('y', -16)
    ...
    >
      ...
      <div className="mt-3 flex flex-wrap items-center gap-4 text-[10px] text-muted-foreground">
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-3 w-5 rounded-full bg-muted-foreground/12" />{' '}
          Global P25&ndash;P90
        </span>
        <span className="flex items-center gap-1.5">
        ...
        <span className="flex items-center gap-1.5">
          <span
            ...
            style={{ background: DOT_ABOVE_P75 }}
          />{' '}
          SLU above P75
        </span>
        <span className="flex items-center gap-1.5">
          <span
            className="inline-block h-3 w-3 rounded-full"
            style={{ background: DOT_MID }}
          />{' '}
          SLU at P50&ndash;P75
        </span>
        <span className="flex items-center gap-1.5">
          <span
            className="inline-block h-3 w-3 rounded-full"
            style={{ background: DOT_BELOW_P50 }}
          />{' '}
          SLU below P50
        </span>
      </div>
        ...
        {rendered && (
          <div>
            ...
            <div className="mt-0.5 text-[11px] text-muted-foreground">
              {rendered.field} &middot; n={rendered.n}
            </div>
